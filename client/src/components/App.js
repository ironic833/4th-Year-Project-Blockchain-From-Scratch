...
import React, { Component } from "react";
import { FormGroup, FormControl, Button, Alert, Table } from 'react-bootstrap';
import { Link } from 'react-router-dom';
import NavBar from "./Usability/Navbar";
import NoInputBlockNavbar from "./Usability/NoInputNavbar";
import { CopyToClipboard } from 'react-copy-to-clipboard';
...
class App extends Component {
    state = { walletInfo: {}, passphrase: '', canSubmit: false, walletHistory: [] };
    ...
    componentDidMount() {
        ...
        fetch(`${document.location.origin}/api/wallet-history`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ walletAddress: this.state.walletInfo.address })
        })
        .then(response => response.json())
        .then(json => this.setState({ walletHistory: json }));
    }
                            ...
                            <tbody>
                                {this.state.walletHistory.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td>{JSON.stringify(item.transaction)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </Table>
